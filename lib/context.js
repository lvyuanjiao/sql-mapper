...
Context.prototype.value = function(path, type) {
    ...
    if (val === undefined) {
        throw new Error('#' + this.entry + ' ' + path.join('.') + ' is unavailable in context.');
    }
    ...
    if (!handler) {
        throw new Error('#' + this.entry + ' handler ' + type + ' is unavailable.');
    }

    try {
        return handler(val);
    } catch (e) {
        throw new Error('#' + this.entry + ' ' + path.join('.') + ' ' + e.message);
    }
};
