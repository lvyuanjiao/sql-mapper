...
exports.query = function(name, args, map, cache, plugins) {
  return {
    'name': name,
    'args': args,
    'plugins': plugins,
...
exports.combi = function(struct, child) {
  if (!struct) {
    return {
      'id': [],
      'result': {},
  ...
  if (child.type) {
    if (child.type === 'id') {
      struct.id.push(child.property);
    }
    struct[table[child.type]][child.property] = child.value;
  }
  return struct;
};

exports.maps = function(original) {
  return original.association;
};
