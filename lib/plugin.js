...
Chain.prototype.onSql = function(params, done) {
  var self = this;
  async.eachSeries(self.chain, function(filter, next) {
    var handle = filter.handler.onSql;
    if(!handle) {
      return next();
    }
    handle(params, filter, next);
  }, done);
};

Chain.prototype.onQuery = function(params, done) {
  var self = this;
  async.eachSeries(self.chain, function(filter, next) {
    var handle = filter.handler.onQuery;
    if(!handle) {
      return next();
    }
    handle(params, filter, next);
